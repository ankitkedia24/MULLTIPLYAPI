...
- `syncId` (item, SKU **and every selling unit** — Mulltiply spec correction
  12-08-2026, needed by their inventory-sync API) and `sellerSKU` = the book's
  ISBN / internal item code — stable and globally unique. `barCode` is set only
  when the code is a real ISBN-10/13. `productCode` = the ERP's Additional Book
  Code (dealer code, e.g. "D659"), falling back to the ISBN when no code is
  maintained.
