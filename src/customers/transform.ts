...
export function stateCode(state: string): string {
  ...
  if (STATE_CODES[normalised]) return STATE_CODES[normalised];
  if (key.length === 2 && CODE_SET.has(key)) return key;
  return "";
}
 ...
 */
export function customerSyncId(customerCode: string): string {
...
function address(
  ...
  return {
    ...
    ...(kind === "shipping" ? { addressType: "SHIPPING_ADDRESS" as const } : {}),
  };
}
 ...
 */
export function customerToRetailer(row: CustomerFeedRow, _cfg: Config): CustomerTransformResult {
  const customerCode = clean(row.customer_code);
  const customerName = clean(row.customer_name);
  const warnings: string[] = [];

  if (!customerCode) return { status: "skip", customerCode, customerName, reason: "no customer code" };
  if (!customerName) return { status: "skip", customerCode, customerName, reason: "no customer name" };
  ...
  if (email && !validEmail) warnings.push("email_dropped_invalid");

  const gst = clean(row.gstin).toUpperCase();
  const validGst = GSTIN_RE.test(gst) ? gst : "";
  if (gst && !validGst) warnings.push("gstin_dropped_invalid");

  const code = stateCode(clean(row.state));
  if (clean(row.state) && !code) warnings.push("state_code_unknown");
  if (!clean(row.address)) warnings.push("no_address");
  if (!clean(row.pin_code)) warnings.push("no_pincode");
  ...
  const retailer: MulltiplyRetailer = {
    name: customerName,
