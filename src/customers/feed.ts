...
import type { CustomerFeedRow } from "./types.js";
...
export interface FetchCustomersOptions {
  /** ISO timestamp — only rows whose changed_at >= since (incremental). */
  since?: string;
  /** Cap the number of rows (previews / tests). */
  limit?: number;
  /** One customer by ERP code. */
  customerCode?: string;
}
...
): Promise<CustomerFeedRow[]> {
  const rows: CustomerFeedRow[] = [];
  let offset = 0;

  for (;;) {
    const pageLimit =
      opts.limit !== undefined ? Math.min(PAGE_SIZE, opts.limit - rows.length) : PAGE_SIZE;
    ...
    if (opts.since) params.set("changed_at", `gte.${opts.since}`);
    if (opts.customerCode) params.set("customer_code", `eq.${opts.customerCode}`);
    ...
    const page = (await res.json()) as CustomerFeedRow[];
    rows.push(...page);
