 ...
 */
export function validateItems(items: MulltiplyItem[]): ValidationOutcome {
  const valid: MulltiplyItem[] = [];
  const invalid: InvalidItem[] = [];
  const seenItemSyncIds = new Set<string>();
  ...
  for (const item of items) {
    ...
    for (const sku of item.skus) seenSkuSyncIds.add(sku.syncId);
    valid.push(item);
  }
