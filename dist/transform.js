 ...
 */
export function bookToItem(row, cfg) {
    ...
    if (!subCategory)
        warnings.push("sub_category_fallback");
    const binding = clean(row.binding) || "PB";
    if (!clean(row.binding))
        warnings.push("binding_defaulted");
    const language = clean(row.language) || "English";
    if (!clean(row.language))
        warnings.push("language_defaulted");
    ...
    const description = clean(row.short_description) ||
    ...
    if (stock < 0)
        warnings.push("negative_stock_clamped");
    const imageUrl = clean(row.cover_image_url);
    const image = imageUrl
        ? { source: "OTHERS", links: [imageUrl] }
        : null;
    const skuNickName = bookName.length > SKU_NICKNAME_MAX
