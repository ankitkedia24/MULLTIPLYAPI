 ...
 */
export async function sendWithRetry(url, method, body, cfg, fetchImpl) {
 ...
 * is 0- or 1-based).
 */
export async function pushBatch(items, batchNo, cfg, fetchImpl = fetch) {
    const url = `${cfg.MULLTIPLY_BASE_URL}/v2/items/sync-data`;
    const { res, attempts, lastError } = await sendWithRetry(url, "PUT", JSON.stringify(items), cfg, fetchImpl);
    if (!res) {
        return {
            kind: "items",
            batchNo,
 ...
 */
export async function pushStockBatch(updates, batchNo, cfg, fetchImpl = fetch) {
    const url = `${cfg.MULLTIPLY_BASE_URL}/v2/godowns/thirdparty-stock-sync`;
    const body = JSON.stringify(updates.map((u) => ({
        syncId: `${cfg.SYNC_STOCK_SYNCID_PREFIX}${u.isbn}`,
        inventoryQuantity: u.quantity,
    })));
    const { res, attempts, lastError } = await sendWithRetry(url, "POST", body, cfg, fetchImpl);
    if (!res) {
        return {
            kind: "stock",
            batchNo,
    ...
    const rowErrors = Array.isArray(raw)
        ? raw.map((e) => {
            ...
            const reason = entry?.["reason"] ?? entry?.["message"] ?? entry?.["error"] ?? JSON.stringify(entry);
            return {
...
function mapRowErrors(response, items) {
    const errors = response?.data?.errors;
    if (!errors?.length)
        return [];
    return errors.map((e) => {
        // try the index as given, then off-by-one, then itemName match
        const byIndex = items[e.row] && items[e.row].itemName === e.itemName
            ? items[e.row]
...
//# sourceMappingURL=mulltiply-client.js.map