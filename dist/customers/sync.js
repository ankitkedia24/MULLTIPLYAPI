...
let inFlight = null;
export function getCustomerInFlight() {
    return inFlight;
}
...
export async function writeCustomerReport(dataDir, report) {
...
export async function readLatestCustomerReport(dataDir) {
    try {
        ...
        if (!latest)
            return null;
        return JSON.parse(await readFile(join(runsDir(dataDir), latest), "utf8"));
    }
    catch {
        return null;
    }
}
export function formatCustomerSummary(report) {
 ...
 */
export async function runCustomerSync(cfg, opts, deps = {}) {
    if (inFlight)
        throw new CustomerSyncBusyError(inFlight.runId);
    ...
    try {
        ...
        if (dryRun) {
            ...
            for (const [i, batch] of batches.entries()) {
                const result = await pushRetailerBatch(batch, i + 1, cfg, deps.fetchImpl);
                ...
                if (i + 1 < batches.length && cfg.SYNC_BATCH_DELAY_MS > 0)
        ...
        if (!dryRun && !opts.customerCode && !opts.limit && fullySuccessful) {
            ...
            if (opts.mode === "full")
                customer.lastFullSyncAt = report.startedAt;
        }
        if (!dryRun) {
            customer.lastRun = {
                runId: report.runId,
...
function deriveStatus(report) {
    ...
    if (failedBatches > 0 || report.totals.rowErrors > 0)
