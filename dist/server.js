...
import { CustomerSyncBusyError, getCustomerInFlight, readLatestCustomerReport, runCustomerSync, } from "./customers/sync.js";
...
app.post("/sync/customers", async (req, reply) => {
    ...
    if (inFlight)
        return reply.code(409).send({ error: "customer sync already running", inFlight });
    const runId = new Date().toISOString().replace(/[:.]/g, "-").replace("Z", "");
...
app.get("/preview/customers", async (req, reply) => {
    ...
    for (const row of rows) {
        ...
        if (r.status === "skip")
            skipped.push({ customerCode: r.customerCode, customerName: r.customerName, reason: r.reason });
        else {
